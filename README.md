...
AI-assisted recipe discovery app: React (Vite + TypeScript) frontend and a small Node API that proxies **Spoonacular** and optional **OpenAI** calls so secrets stay off the client.

## Prerequisites

- Node.js 20+ (CI uses 22)
- [Spoonacular](https://spoonacular.com/food-api) API key
- Optional: OpenAI (or compatible) key for recipe copilot and natural-language search

## Quick start

1. Set **`SPOONACULAR_API_KEY`** in either the **repo root** `.env` or `apps/api/.env` (see [`.env.example`](./.env.example)). The API loads both; `apps/api/.env` wins if a variable is set in both. A root **`REACT_APP_API_KEY`** from the old CRA setup is still accepted as the Spoonacular key. Add `OPENAI_API_KEY` if you want AI features.
...
- [`apps/api`](./apps/api): Hono server — `/api/recipes/*`, `/api/ai/recipe-assist`, `/api/ai/search-parse`, CORS, simple rate limits on AI routes.
